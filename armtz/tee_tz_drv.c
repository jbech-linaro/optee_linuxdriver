...
static struct tee_shm *handle_rpc_alloc(struct tee_tz *tee_tz, size_t size)
{
	struct tee_rpc_alloc rpc_alloc;

	rpc_alloc.size = size;
	tee_supp_cmd(tee_tz->tee, TEE_RPC_ICMD_ALLOCATE,
		     &rpc_alloc, sizeof(rpc_alloc));
	return rpc_alloc.shm;
}

static void handle_rpc_free(struct tee_tz *tee_tz, struct tee_shm *shm)
{
	...
	if (!shm)
		return;
	rpc_free.shm = shm;
	tee_supp_cmd(tee_tz->tee, TEE_RPC_ICMD_FREE, &rpc_free, sizeof(rpc_free));
}
...
static u32 handle_rpc(struct tee_tz *tee_tz, struct smc_param *param)
{
	...
	case TEESMC_ST_RPC_FUNC_ALLOC_PAYLOAD:
		shm = handle_rpc_alloc(tee_tz, param->a1);
